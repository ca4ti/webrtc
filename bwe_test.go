		...
		t.Run(tc.name, func(t *testing.T) {
			...
			go func() {
				...
				for {
					...
					case b := <-receivedMetrics:
						bytesReceived += b
					case <-ticker.C:
						log.Infof("received %v bit/s", bytesReceived*8)
						bytesReceived = 0
					}
				}
			}()
			onTrack := func(trackRemote *TrackRemote, rtpReceiver *RTPReceiver) {
				for {
					...
					if err == io.EOF {
						log.Info("trackRemote.ReatRTP received EOF")
						return
					}
					if err != nil {
						log.Errorf("trackRemote.ReatRTP returned unexpected error: %v", err)
						return
					}
			...
			log.Info("setup forward")

			mss := []*mediaSender{}
			for _, forward := range tc.forward {

				spc, cc, err := forward.sender.createPeer(leftRouter.Router, cc.LeakyBucketPacerFactory, cc.GCCFactory)
				assert.NoError(t, err)

				ms := newMediaSender(log, spc, cc)
			...
			log.Info("setup backward")

			for _, backwardPair := range tc.backward {

				spc, cc, err := backwardPair.sender.createPeer(rightRouter.Router, cc.LeakyBucketPacerFactory, cc.GCCFactory)
				assert.NoError(t, err)
				ms := newMediaSender(log, spc, cc)

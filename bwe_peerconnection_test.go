...
func (s *sender) createPeer(router *vnet.Router, pacer cc.PacerFactory, bwe cc.BandwidthEstimatorFactory) (*PeerConnection, cc.BandwidthEstimator, error) {
	...
	if err := router.AddNet(sendNet); err != nil {
		return nil, nil, fmt.Errorf("failed to add sendNet to routerA: %w", err)
	}
	...
	ccInterceptor, err := cc.NewControllerInterceptor(pacer, bwe)
	if err != nil {
		return nil, nil, err
	}
	...
	estimator, err := ccInterceptor.GetBandwidthEstimator("")
	if err != nil {
		return nil, nil, err
	}

	return offerPeerConnection, estimator, nil
}
